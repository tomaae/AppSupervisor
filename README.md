...
When Windows exposes BLE advertisement events, the grid shows raw signal strength and a broad **Very near**, **Near**, **Far**, or **Very far** estimate based on the strongest signal observed during that scan. The estimate is intentionally approximate because walls, interference, transmit power, and antenna orientation affect RSSI. Manufacturer-specific advertisement company IDs are resolved through a bundled offline snapshot of the Bluetooth assigned-numbers registry and displayed as a **Company hint**. That hint identifies the owner of the advertising payload and may differ from the consumer-facing device brand; devices that do not advertise manufacturer data remain unidentified rather than receiving a guess from a potentially randomized BLE address.
